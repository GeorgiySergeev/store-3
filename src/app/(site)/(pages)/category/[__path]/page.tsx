...
import MainCategoryWithFilter from '@/components/Category/MainCategoryWithFilter';
import { getChildrenCategories } from '@/constants/categories';
import { store } from '@/redux/store';
import OneCategoryPage from '@/components/Category/CategotyList';
import { createClient } from '@/utils/supabase/client';
import CategoryList from '@/components/Category/CategotyList';
...
import Dropdown from '@/components/Header/Navigation';
...
const CategoryPage = () => {
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      ...
      // First, fetch the main category by ID
      ...
      const getAllSubcategoryIds = async (parentId) => {
        ...
        const subcatIds = subcats.map((subcat) => subcat._id);

        // Recursively get IDs of deeper level subcategories
        const deeperSubcatIds = await Promise.all(subcatIds.map((id) => getAllSubcategoryIds(id)));

        // Flatten the array of arrays and combine with current level IDs
        return [...subcatIds, ...deeperSubcatIds.flat()];
      };
      ...
      const { data: directSubcategories, error: directSubcatsError } = await supabase
        .from('categories')
        .select('*')
        .eq('_parentId', categoryId);

      if (directSubcatsError) {
      ...
      // Get total count of items for pagination across all categories
      ...
      // Fetch items for current page from all categories
    ...
    fetchCategory();
  }, [categoryId, currentPage, supabase]);
  ...
  return (
    <div className="">
      <Breadcrumb title={'Всі товари'} pages={['категорії', 'магазин']} />
